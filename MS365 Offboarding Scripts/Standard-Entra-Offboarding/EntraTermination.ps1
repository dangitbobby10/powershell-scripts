...
function Import-ModuleIfNeeded {
    ...
    if (-not $loadedModules.ContainsKey($ModuleName)) {
...
function Connect-ToGraph {
    try {
        ...
        Import-ModuleIfNeeded "Microsoft.Graph"
...
function Export-UserMemberships {
    ...
    try {
        ...
        # Get username from UPN and format date in friendly format (MM-DD-YYYY)
        $username = $UserPrincipalName.Split('@')[0]
        $friendlyDate = Get-Date -Format "MM-dd-yyyy"
        $exportFile = Join-Path $ExportPath "$username`_offboarding-$friendlyDate.csv"
...
function Export-UserLicenses {
    ...
    try {
        ...
        $exportFile = if ($ExistingCsvPath -and (Test-Path $ExistingCsvPath)) {
            ...
            Join-Path $ExportPath "$($UserPrincipalName.Split('@')[0])-Licenses-$timestamp.csv"
        }
...
# Remove script block and make execution direct
...
if ($args -contains "-CreateTemplate") {
    ...
    # Disconnect from services before exiting
    Disconnect-FromGraph
